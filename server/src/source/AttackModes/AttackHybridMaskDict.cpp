...
CAttackHybridMaskDict::CAttackHybridMaskDict(PtrJob job, PtrHost &host, uint64_t seconds, CSqlLoader *sqlLoader)
...
bool CAttackHybridMaskDict::makeWorkunit()
{
	...
	Tools::printDebugHost(Config::DebugType::Log, m_job->getId(), m_host->getBoincHostId(),
			"In mask, there are %" PRIu64 " total passwords\n", m_job->getHcKeyspace());
	Tools::printDebugHost(Config::DebugType::Log, m_job->getId(), m_host->getBoincHostId(),
	...
	try
	{
		...
		{
			std::string maskLine = makeConfigLine("mask", "String", workunitMask->getMask());
			f<<maskLine;
			Tools::printDebug("%s", maskLine.c_str());
		}

		uint64_t prevJobIndex = m_job->getCurrentIndex();

		/** Check hybrid workunit type */
		/** Mask is already fragmented, continue fragmenting */
		if (m_workunit->getStartIndex2() != 0)
		{
			Tools::printDebugHost(Config::DebugType::Log, m_job->getId(), m_host->getBoincHostId(),
					"Mask is already fragmented, we need to continue\n");
			...
			if(!dict2_file->WritePasswordsTo(1, outputDictPath))
			{
				...
				if (!m_workunit->isDuplicated())
				{
						...
						m_job->updateIndex(prevJobIndex + 1);
				}
				return true;
			}
			...
			/** Check if we reach the end of password keyspace in mask */
		...
		else if (m_workunit->getHcKeyspace() > (0.5f * m_job->getHcKeyspace()))
		{
			...
			if (!m_workunit->isDuplicated())
			{
				...
				m_job->updateIndex(prevJobIndex + secDictKeyspace);
			}

			/** Exact number of passwords in workunit
			* @warning Not really passwords, just hc index units
			**/
	...
	retval = create_work(
			wu,
			Config::inTemplatePathHybridMaskDict,
			path,
...
bool CAttackHybridMaskDict::generateWorkunit()
{
	Tools::printDebugHost(Config::DebugType::Log, m_job->getId(), m_host->getBoincHostId(),
			"Generating hybrid (mask+dict) workunit ...\n");
	...
	/**
	 * Create the workunit
	 * @warning We save number of real passwords to hc_keyspace as we don't work with hc_indexes in hybrid attack
	 */
	m_workunit = CWorkunit::create(m_job->getId(), m_host->getId(), m_host->getBoincHostId(), currentDict->getCurrentIndex(),
						 m_job->getCurrentIndex2(), passCount, currentMask->getId(), currentDict->getId(), false, 0, false);
	/**
